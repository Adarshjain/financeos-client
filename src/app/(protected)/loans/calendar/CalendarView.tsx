...
export function CalendarView() {
  ...
  const { data, error } = useQuery({
    queryKey: keys.lendings.obligations(months),
    queryFn: async () =>
      (
        await api.GET('/api/v1/obligations/upcoming', {
          params: { query: { months } },
        })
      ).data! as ObligationsResponse,
  ...
  const monthlyGroups = upcomingItems.reduce(
    (acc, item) => {
    ...
    <div className="pb-20 p-3 sm:p-6 space-y-3 max-w-7xl mx-auto w-full min-w-0 overflow-x-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Timeline of upcoming loan EMIs and P2P expected returns over your
            chosen schedule window
          </p>
        </div>
      </div>
      ...
      {error && (
        <div className="bg-rose-50/60 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 rounded-xl p-3 text-xs text-rose-600 dark:text-rose-400">
          Failed to load obligations. Please try again.
        </div>
      )}
        ...
        <div className="bg-rose-50/60 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/50 rounded-xl overflow-hidden shadow-sm">
          <div className="p-4 border-b border-rose-200 dark:border-rose-900/50 flex items-center justify-between">
            <div className="flex items-center gap-2 text-rose-600 dark:text-rose-400 font-bold text-sm">
              <AlertCircle className="h-4 w-4" /> Overdue Obligations (
              {overdueItems.length})
            </div>
          </div>
          <div className="divide-y divide-rose-100 dark:divide-rose-900/40">
            {overdueItems.map((item, idx) => (
              <div
                key={idx}
                className="p-3.5 flex items-center justify-between hover:bg-rose-100/40 transition-colors text-xs"
              >
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <Badge
                      variant="destructive"
                      className="capitalize text-2xs px-1.5 py-0"
                    >
                      Overdue
                    </Badge>
                  </div>
                  ...
                  >
                    <Link
                      href={
                        item.type === 'emi'
                          ? `/loans/${item.loanId}`
                          : `/loans/lendings/${item.counterpartyId}`
                      }
                    >
      ...
      <div className="space-y-3">
        {Object.keys(monthlyGroups).length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl p-12 text-center text-slate-400 text-xs">
            ...
            No upcoming obligations scheduled within the selected {months}-month
            window.
          </div>
        ) : (
            ...
            >
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-3.5 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors text-xs"
                  >
                    <div className="flex items-center gap-3">
