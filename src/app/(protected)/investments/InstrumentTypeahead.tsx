...
import { searchInstruments } from '@/actions/investments';
  ...
  return (
    <div ref={containerRef} className="relative space-y-1">
        ...
        <div className="absolute left-0 right-0 z-50 mt-1 max-h-60 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 shadow-lg p-1 space-y-1">
          {loading ? (
            ...
            results.map((inst) => (
              <button
                ...
                className="w-full text-left p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-900 flex items-center justify-between transition-colors"
              >
                <div className="space-y-0.5 truncate">
                  ...
                  <div className="text-[10px] text-slate-400 flex items-center gap-1">
                    <span>{inst.symbol || 'No symbol'}</span>
                    {inst.exchange ? <span>• {inst.exchange}</span> : null}
                  </div>
                </div>
          ...
          <div className="border-t border-slate-100 dark:border-slate-800 pt-1">
            <button
              ...
              className="w-full text-left px-3 py-2 text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 rounded-md flex items-center gap-1.5 transition-colors"
            >
      ...
      <CreateInstrumentDialog
        ...
        onCreated={(newInst) => handleSelect(newInst)}
      />
    </div>
  );
}
