...
export function TradebookSection({ initialData, brokerAccounts, accounts }: TradebookSectionProps) {
  ...
  const getTypeBadge = (type: string | undefined) => {
    switch (type?.toLowerCase()) {
      case 'buy':
        return (
          <Badge variant="outline" className="bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 font-bold text-2xs uppercase px-2 py-0.5">
            BUY
          </Badge>
        );
      case 'sell':
        return (
          <Badge variant="outline" className="bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 border-rose-200 dark:border-rose-800 font-bold text-2xs uppercase px-2 py-0.5">
            SELL
          </Badge>
        );
      default:
        return (
          <Badge variant="outline" className="font-bold text-2xs uppercase px-2 py-0.5">
  ...
  const getSettlementBadge = (settlementType: string | undefined) => {
    if (settlementType?.toLowerCase() === 'intraday') {
      return (
        <Badge variant="outline" className="bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border-amber-200 dark:border-amber-800 font-bold text-2xs uppercase px-1.5 py-0" title="Intraday (MIS)">
          MIS
        </Badge>
      );
    }
    return (
      <Badge variant="outline" className="bg-sky-100 text-sky-800 dark:bg-sky-950 dark:text-sky-300 border-sky-200 dark:border-sky-800 font-bold text-2xs uppercase px-1.5 py-0" title="Delivery (CNC)">
        CNC
      </Badge>
    );
  };
                ...
                <CardContent className="p-3 sm:p-3.5 space-y-1 flex-1 flex flex-col justify-between pt-0">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-2xs text-slate-400 tabular-nums ml-1">/ unit</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
