...
export function TradebookSection({investmentTransactions, brokerAccounts, accounts}: TradebookSectionProps) {
  ...
  const getTypeBadge = (type: string | undefined) => {
    switch (type?.toLowerCase()) {
      case 'buy':
        return (
            <Badge
                className="bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 font-bold border-0 text-[10px]">
              BUY
            </Badge>
        );
      case 'sell':
        return (
            <Badge
                className="bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300 font-bold border-0 text-[10px]">
              SELL
            </Badge>
        );
      default:
  ...
  return (
      <Card
        ...
        <CardHeader className="p-3.5 py-2 border-b border-slate-100 dark:border-slate-800">
          <div className="text-base font-bold flex sm:flex-row sm:items-center justify-between gap-3">
            ...
            <Select value={selectedBrokerFilter} onValueChange={setSelectedBrokerFilter}>
              <SelectTrigger
                  className="h-8 text-xs bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-800 rounded-lg font-semibold w-[150px]">
                  ...
                  {filteredTransactions.map((tx) => (
                      <div key={tx.id} className="p-3.5 space-y-2">
                        <div className="flex items-center justify-between text-xs">
                          <span
                              className="text-slate-500 dark:text-slate-400 font-mono">{formatDate(tx.tradeDate)}</span>
                    ...
                    <TableBody>
                      {filteredTransactions.map((tx) => (
                          <TableRow key={tx.id} className="border-slate-100 dark:border-slate-800/60">
                            <TableCell
                                className="py-2.5 text-xs text-slate-600 dark:text-slate-400 tabular-nums whitespace-nowrap">
                              {formatDate(tx.tradeDate)}
                            </TableCell>
                            <TableCell
                                className="py-2.5 text-xs text-slate-600 dark:text-slate-400 font-medium whitespace-nowrap">
                              {getBrokerName(tx)}
                            </TableCell>
